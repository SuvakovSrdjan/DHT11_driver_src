...
void timer_reached_value(int value) {
  int i = cnt;
  int j = cnt;
  while ((j - i) < value) {
    j = cnt;
...
void SendInitSequenceAndReciveData(char pin) {
  ...
  timer_reached_value(TIMER_18_ms_SEC);

  SetInternalPullUpDown(pin, PULL_UP);
  timer_reached_value(TIMER_40_us_SEC);
  ...
  for (i = 0; i < 85; i++) {
    ...
    while (GetGpioPinValue(pin) == laststate) {
      counter++;
      timer_reached_value(TIMER_1_us_SEC);
      ;
      if (GetGpioPinValue(pin) == PULL_UP) {
        printk("STATE:PULL_UP");
      }
      if (counter == 225) {
        break;
      }
...
int DHT11_driver_init(void) {
  ...
  */
  ...
  kt = ktime_set(TIMER_SEC, TIMER_1_us_SEC);
